...
import { DeveloperListPage as ShowCaseList } from 'pages/developers';
...
function DashboardPage() {
  ...
  const recruiterStat = useSWR<PaginatedInterface<RecruiterInterface>>(
    () => `/recruiters`,
    () => getRecruiters({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Recruiters',
      result: recruiterStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`developers`} passHref legacyBehavior>
                  <Button
